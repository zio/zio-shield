...
class ZioShieldScalafixExtension(fullClasspath: List[Path]) {
  ...
  def semanticDocumentFromPath(doc: SyntacticDocument, path: Path): Either[Throwable, SemanticDocument] =
    for {
      doc <- Try {
  ...
  def semanticDocumentFromTextDocument(doc: SyntacticDocument, textDoc: TextDocument): SemanticDocument =
